...
/*
 ...
 $info : 画像ファイルのときgetimagesize()の'size'
         ...
         URLのとき : URLそのもの
*/
	$title = $url = $url2 = $info = $resize_info = '';
	$width = $height = 0;
	$matches = array();
	...
	if ($is_image) {
		...
		if ($width && $height) {
			 ...
			 $resize_info = "&amp;width=$width&amp;height=$height";
		}
	}
	...
	if ($is_image) { // 画像
		if ($is_url || !PLUGIN_REF_RESIZE_IMAGE) {
			$params['_body'] = "<img class=\"lazyload\" src=\"image/spacer.png\" data-src=\"$url\" alt=\"$title\" title=\"$title\" $info/>";
		} else {
			$params['_body'] = "<img class=\"lazyload\" src=\"image/spacer.png\" data-src=\"$resize_url$resize_info\" alt=\"$title\" title=\"$title\" />";
		}
